...
function woocommerce_payu_latam_gateway() {
	...
	class WC_Payu_Latam extends WC_Payment_Gateway {
		...
		function receipt_page($order){
			echo '<p class="message">'.__('Gracias por su pedido, de clic en el botón que aparece para continuar el pago con PayU Latam.', 'payu_latam').'</p>';
		...
		public function get_params_post($order_id){
			...
			$signature = md5($this -> api_key . '~' . $this -> merchant_id . '~' . $order_id . '~' . $amount . '~' . $currency );
			$description = "";
