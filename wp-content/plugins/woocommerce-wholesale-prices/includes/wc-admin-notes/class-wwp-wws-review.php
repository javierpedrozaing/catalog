	...
	public function review_wws() {
		...
		if( get_option( WWP_REVIEW_REQUEST_RESPONSE ) === 'never-show' || get_option( WWP_REVIEW_REQUEST_RESPONSE ) === 'review-later' ) {
			...
			try {
				...
				$note_content = __(
					'We notice you\'ve been using Wholesale Suite for a couple of weeks now. We\'d love to get your review on our plugin listing! Your review helps give others the confidence to try our plugin.',
					'woocommerce-wholesale-prices'
				);
	
				$note = new WC_Admin_Note();
				$note->set_title( __( 'Review Wholesale Suite', 'woocommerce-wholesale-prices' ) );
				$note->set_content( $note_content );
				$note->set_content_data( (object) array() );
				$note->set_type( WC_Admin_Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
				$note->set_name( self::NOTE_NAME );
				$note->set_source( 'woocommerce-admin' );
				$note->add_action( 'review-wws', __( 'Review Wholesale Suite', 'woocommerce-wholesale-prices' ), $review_link , WC_Admin_Note::E_WC_ADMIN_NOTE_ACTIONED, true );
				$note->save();
